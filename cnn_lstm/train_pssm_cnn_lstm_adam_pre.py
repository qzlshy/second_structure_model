...
cnn=cnn_lstm.cnn_v(x)
cnn_rsp=tf.reshape(cnn,[1,-1,256])
...
(outputs_fw,outputs_bw), output_states=tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell,lstm_bw_cell,inputs=cnn_rsp,dtype=tf.float32)
outputs=tf.concat([outputs_fw, outputs_bw],axis=-1)
outputs_reshape=tf.reshape(outputs,[-1,512])
net = slim.fully_connected(outputs_reshape, 3, activation_fn=None, scope='fc2')
